...
TEST_CASE("Parse args", "[cli]") {
    ...
    REQUIRE(args.source == ticker::types::DataSource::Bybit);
    REQUIRE(args.mode == ticker::types::Mode::Backtest);
...
TEST_CASE("Use default args", "[cli]") {
    ...
    REQUIRE(args.source == ticker::cli::DEFAULT_SOURCE);
    REQUIRE(args.mode == ticker::cli::DEFAULT_MODE);
...
TEST_CASE("Invalid args", "[cli]") {
    ...
    const int empty_window_argc = 3;
    const char *empty_window_argv[empty_window_argc] = {"./tests", "-w", "0"};

    REQUIRE_THROWS(
        ticker::cli::parse_args(empty_window_argc, empty_window_argv)
    );
}
